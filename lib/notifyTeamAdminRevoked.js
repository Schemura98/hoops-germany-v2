...
export async function notifyTeamAdminRevoked(playerId, team, neuerAdmin, optionen = {}) {
  try {
    ...
    const message = optionen.ohneNachfolger
      ...
      : // ⚠️ „wurde an X übertragen", NICHT „liegt jetzt bei X" (Nachtrag Nele
        // auf Kais Anmerkung 5, 14.08.2026): Die Nachricht friert den Namen
        // beim Schreiben ein. Bei A→B→C hielte A eine Notiz „liegt JETZT bei
        // B" – Präsens, eine Woche später falsch, und schärfer als
        // gewöhnliches Veralten, weil der Name hier der Rückweg IST.
        // „wurde übertragen" ist eine Aussage über einen Zeitpunkt und bleibt
        // wahr. „übertragen" beschreibt außerdem nur die Handlung der
        // Verwaltung und erfindet – anders als „übergeben"/„korrigiert" –
        // keine Vorgeschichte.
        // Der zweite Halbsatz darf im Präsens bleiben: Bekommt er die Rechte
        // zurück, erzeugt das eine eigene `team_admin_granted`-Nachricht und
        // korrigiert den Eindruck. Für den Namen gibt es diese Selbstkorrektur
        // nicht – deshalb muss nur dort die Zeit stimmen.
        // ⚠️ Fallback im AKKUSATIV („jemand anderen"), sonst ist der Satz
        // grammatisch falsch – gefangen von Nele.
        `Die Verwaltung von ${team.teamName} wurde an ${nachfolger || "jemand anderen"} ` +
        `übertragen – die Admin-Rechte für den Verein hast du damit nicht mehr, Mitglied ` +
        `von ${team.teamName} bleibst du. Eingetragen hat das die Verwaltung von Hoops ` +
        `Germany; wenn das nicht stimmt, schreib uns über „Kontakt".`;
