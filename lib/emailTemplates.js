...
export function welcomeEmail({ firstName, baseUrl }) {
  ...
  const bodyHtml = `
    <p style="margin:0 0 4px;color:#0f172a;font-size:15px;font-weight:700;text-align:center">Leg gleich los:</p>
    ${linkCard(`${baseUrl}/player/edit-profile`, "Profil ausfüllen", "Position, Größe, Verein und ein Bild – damit Teams dich finden.")}
    ${linkCard(`${baseUrl}/teams`, "Team finden & beitreten", "Stöbere durch Vereine in deiner Region und stell eine Beitrittsanfrage – oder gründe dein eigenes Team.")}
  ...
  return {
    ...
    html: emailLayout({
      ...
      intro:
        "Schön, dass du dabei bist! Bei Hoops Germany findest du dein Team, verfolgst Spiele & Ergebnisse und vernetzt dich mit der Basketball-Community in ganz Deutschland.",
      bodyHtml,
...
export function pendingResultEmail({ teamName, opponentName, matchDate, baseUrl }) {
  ...
  return {
    ...
    html: emailLayout({
      ...
      ctaUrl: `${baseUrl}/team/admin?tab=ergebnisse`,
      footerNote:
    ...
    text:
      ...
      `Jetzt eintragen: ${baseUrl}/team/admin?tab=ergebnisse\n`,
  };
}

// ---------------------------------------------------------------------------
// 02b – Beitrittsanfrage an den Team-Admin
// kind="slot" → Spieler beansprucht einen Kaderplatz (Bestätigung im Kader-Tab);
// kind="join" → allgemeine Beitrittsanfrage (Bestätigung im Anfragen-Tab).
// ---------------------------------------------------------------------------
export function joinRequestEmail({ teamName, playerName, position, kind = "join", baseUrl }) {
  const isSlot = kind === "slot";
  const tab = isSlot ? "kader" : "anfragen";
  const title = isSlot
    ? `${playerName} möchte einen Kaderplatz übernehmen`
    : `${playerName} möchte deinem Team beitreten`;
  const bodyHtml = `
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e8ebf0;border-radius:12px;overflow:hidden">
      <tr>
        <td style="padding:10px 16px;border-bottom:1px solid #eef1f5;color:#64748b;font-size:13px;font-weight:600;width:110px">Spieler</td>
        <td style="padding:10px 16px;border-bottom:1px solid #eef1f5;color:#0f172a;font-size:14px;font-weight:700">${playerName}</td>
      </tr>
      <tr>
        <td style="padding:10px 16px;border-bottom:1px solid #eef1f5;color:#64748b;font-size:13px;font-weight:600">Team</td>
        <td style="padding:10px 16px;border-bottom:1px solid #eef1f5;color:#0f172a;font-size:14px">${teamName}</td>
      </tr>
      <tr>
        <td style="padding:10px 16px;color:#64748b;font-size:13px;font-weight:600">Position</td>
        <td style="padding:10px 16px;color:#0f172a;font-size:14px">${position || "–"}</td>
      </tr>
    </table>`;
  return {
    subject: `Neue Beitrittsanfrage: ${playerName} → ${teamName}`,
    html: emailLayout({
      baseUrl,
      accent: "blue",
      ...
      title,
      intro:
        "Es liegt eine neue Anfrage für dein Team vor. Bestätige oder lehne sie in deinem Team-Panel ab – die Person erscheint danach in deinem Kader.",
      bodyHtml,
      ctaText: "Anfrage ansehen",
      ctaUrl: `${baseUrl}/team/admin?tab=${tab}`,
      footerNote:
        "Du erhältst diese E-Mail, weil du ein Team bei Hoops Germany verwaltest. Die Benachrichtigung in der Glocke bleibt davon unberührt.",
    }),
    text:
      `${title}.\n` +
      `Team: ${teamName}${position ? ` · Position: ${position}` : ""}\n` +
      `Anfrage ansehen: ${baseUrl}/team/admin?tab=${tab}\n`,
  };
}

// ---------------------------------------------------------------------------
// 02c – Bestätigung: eingeladener Spieler hat sich registriert und ist im Kader
// (Einladungslink = Autorisierung → direkter Beitritt, kein manuelles Genehmigen).
// ---------------------------------------------------------------------------
export function memberJoinedEmail({ teamName, playerName, position, baseUrl }) {
  const bodyHtml = `
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e8ebf0;border-radius:12px;overflow:hidden">
      <tr>
        <td style="padding:10px 16px;border-bottom:1px solid #eef1f5;color:#64748b;font-size:13px;font-weight:600;width:110px">Spieler</td>
  ...
  return {
    subject: `${playerName} ist deinem Kader beigetreten 🏀`,
    html: emailLayout({
      baseUrl,
      accent: "green",
      badge: "🏀 Neues Mitglied",
      title: `${playerName} ist jetzt in deinem Kader`,
      intro:
        "Deine Einladung wurde angenommen: Der Spieler hat sich registriert und steht ab sofort in deinem Kader. Du musst nichts weiter tun.",
      bodyHtml,
      ctaText: "Kader ansehen",
      ctaUrl: `${baseUrl}/team/admin?tab=kader`,
      footerNote:
        "Du erhältst diese E-Mail, weil du ein Team bei Hoops Germany verwaltest. Die Benachrichtigung in der Glocke bleibt davon unberührt.",
    }),
    text:
      `${playerName} hat deine Einladung angenommen und ist jetzt in deinem Kader (${teamName}${position ? `, ${position}` : ""}).\n` +
      `Kader ansehen: ${baseUrl}/team/admin?tab=kader\n`,
  };
}

// ---------------------------------------------------------------------------
// 02d – Direkte Kader-Einladung an einen bestehenden Account (Annehmen/Ablehnen)
// ---------------------------------------------------------------------------
export function teamInvitePlayerEmail({ teamName, playerName, baseUrl }) {
  const hi = playerName ? `Hi ${playerName.split(" ")[0]},` : "Hi,";
  const bodyHtml = `
    <p style="margin:0 0 14px;color:#475569;font-size:15px;line-height:1.65;text-align:center">${hi}</p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #ffe8d6;border-radius:12px;background:#fff8f2;margin:4px 0 8px">
      <tr><td style="padding:18px;text-align:center">
        <div style="margin-bottom:8px">${contextBadge("🏀 Kader-Einladung")}</div>
        <span style="display:block;color:#0f172a;font-size:18px;font-weight:800">${teamName}</span>
        <span style="display:block;color:#64748b;font-size:13px;margin-top:4px">möchte dich in den Kader aufnehmen.</span>
      </td></tr>
    </table>
    <p style="margin:8px 0 0;color:#64748b;font-size:13px;line-height:1.6;text-align:center">
      Nimm die Einladung an, um Teil der Mannschaft zu werden – der Verein erscheint dann in deinem
      Karriere-Verlauf und du verwaltest Spiele &amp; Statistiken gemeinsam. Über die Glocke kannst du
      jederzeit <strong>annehmen</strong> oder <strong>ablehnen</strong>.
    </p>`;
  return {
    subject: `${teamName} lädt dich in den Kader ein 🏀`,
    html: emailLayout({
      baseUrl,
      accent: "orange",
      badge: "Einladung",
      title: `${teamName} möchte dich im Team`,
      bodyHtml,
      ctaText: "Einladung ansehen",
      ctaUrl: `${baseUrl}/player/newsfeed`,
      footerNote:
        "Du erhältst diese E-Mail, weil ein Team dich bei Hoops Germany in seinen Kader eingeladen hat. Du entscheidest, ob du annimmst.",
    }),
    text:
      `${teamName} möchte dich in den Kader aufnehmen.\n` +
      `Annehmen oder ablehnen kannst du über die Glocke: ${baseUrl}/player/newsfeed\n`,
  };
}
...
export function resultMismatchEmail({
  ...
  return {
    ...
    html: emailLayout({
      ...
      ctaUrl: forSuperAdmin
        ? `${baseUrl}/admin/matches`
        : `${baseUrl}/team/admin?tab=ergebnisse`,
    ...
    text:
      ...
      (forSuperAdmin
        ...
        : `Ergebnis korrigieren: ${baseUrl}/team/admin?tab=ergebnisse\n`),
  };
}
...
// ---------------------------------------------------------------------------
// 08 – Neues Team wartet auf Freigabe (an Super-Admins)
// ---------------------------------------------------------------------------
export function teamPendingEmail({ teamName, founderName, region, baseUrl }) {
  const bodyHtml = `
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e8ebf0;border-radius:12px;overflow:hidden">
      <tr>
        <td style="padding:10px 16px;border-bottom:1px solid #eef1f5;color:#64748b;font-size:13px;font-weight:600;width:110px">Team</td>
        <td style="padding:10px 16px;border-bottom:1px solid #eef1f5;color:#0f172a;font-size:14px;font-weight:700">${teamName}</td>
      </tr>
      <tr>
        <td style="padding:10px 16px;border-bottom:1px solid #eef1f5;color:#64748b;font-size:13px;font-weight:600">Haupt-Admin</td>
        <td style="padding:10px 16px;border-bottom:1px solid #eef1f5;color:#0f172a;font-size:14px">${founderName || "–"}</td>
      </tr>
      <tr>
        <td style="padding:10px 16px;color:#64748b;font-size:13px;font-weight:600">Region</td>
        <td style="padding:10px 16px;color:#0f172a;font-size:14px">${region || "–"}</td>
      </tr>
    </table>`;
  return {
    subject: `Neues Team wartet auf Freigabe: ${teamName}`,
    html: emailLayout({
      baseUrl,
      accent: "orange",
      badge: "Freigabe nötig",
      title: "Neues Team wartet auf Freigabe",
      intro: "Ein Spieler hat ein neues Team gegründet. Es ist erst nach deiner Freigabe öffentlich sichtbar.",
      bodyHtml,
      ctaText: "Im Admin-Panel prüfen",
      ctaUrl: `${baseUrl}/admin/teams`,
      footerNote: "Interne Benachrichtigung · Hoops Germany Team-Freigabe",
    }),
    text: `Neues Team wartet auf Freigabe: ${teamName} (Haupt-Admin: ${founderName || "–"}, Region: ${region || "–"}).\nPrüfen: ${baseUrl}/admin/teams\n`,
  };
}

// ---------------------------------------------------------------------------
// 09 – Team freigegeben (an den Haupt-Admin)
// ---------------------------------------------------------------------------
export function teamApprovedEmail({ teamName, slug, baseUrl }) {
  const bodyHtml = `
    <p style="margin:0 0 4px;color:#0f172a;font-size:15px;font-weight:700;text-align:center">Deine nächsten Schritte:</p>
    ${linkCard(`${baseUrl}/team/admin?tab=kader`, "1. Kader füllen", "Lege deine Spieler an und lade sie per Link, WhatsApp oder E-Mail ein.")}
    ${linkCard(`${baseUrl}/team/admin?tab=spielplan`, "2. Spiele eintragen", "Trag eure Spiele ein und meldet anschließend die Ergebnisse.")}
    ${slug ? linkCard(`${baseUrl}/team/team-detail/${slug}`, "3. Eure Vereinsseite ansehen", "So sieht euer Team jetzt öffentlich aus – teile den Link mit euren Fans.") : ""}`;

  return {
    subject: `Dein Team „${teamName}" ist jetzt freigeschaltet 🏀`,
    html: emailLayout({
      baseUrl,
      accent: "green",
      badge: "🏀 Freigeschaltet",
      title: `„${teamName}" ist live!`,
      intro:
        "Dein Team wurde von unserem Team geprüft und freigegeben – es ist jetzt öffentlich sichtbar. So geht es weiter:",
      bodyHtml,
      ctaText: "Zum Team-Panel",
      ctaUrl: `${baseUrl}/team/admin`,
      footerNote: "Du erhältst diese E-Mail, weil du ein Team bei Hoops Germany gegründet hast.",
    }),
    text: `Gute Nachricht! Dein Team „${teamName}" ist jetzt freigeschaltet und öffentlich sichtbar.\n\nNächste Schritte:\n1. Kader füllen: ${baseUrl}/team/admin?tab=kader\n2. Spiele eintragen: ${baseUrl}/team/admin?tab=spielplan\n${slug ? `3. Vereinsseite: ${baseUrl}/team/team-detail/${slug}\n` : ""}`,
  };
}

// ---------------------------------------------------------------------------
// 10 – Liga-Zuordnungsanfrage wartet auf Freigabe (an Super-Admins + info@)
// ---------------------------------------------------------------------------
export function leagueChangeRequestEmail({
  teamName,
  currentLeagueName,
  requestedLeagueName,
  requesterName,
  ...
  const bodyHtml = `
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e8ebf0;border-radius:12px;overflow:hidden">
      <tr>
        <td style="padding:10px 16px;border-bottom:1px solid #eef1f5;color:#64748b;font-size:13px;font-weight:600;width:130px">Team</td>
        <td style="padding:10px 16px;border-bottom:1px solid #eef1f5;color:#0f172a;font-size:14px;font-weight:700">${teamName}</td>
      </tr>
      <tr>
        <td style="padding:10px 16px;border-bottom:1px solid #eef1f5;color:#64748b;font-size:13px;font-weight:600">Aktuelle Liga</td>
        <td style="padding:10px 16px;border-bottom:1px solid #eef1f5;color:#0f172a;font-size:14px">${currentLeagueName || "– keine –"}</td>
      </tr>
      <tr>
        <td style="padding:10px 16px;${note ? "border-bottom:1px solid #eef1f5;" : ""}color:#64748b;font-size:13px;font-weight:600">Gewünschte Liga</td>
        <td style="padding:10px 16px;${note ? "border-bottom:1px solid #eef1f5;" : ""}color:#0f172a;font-size:14px;font-weight:700">${requestedLeagueName}</td>
      </tr>
      ${
        note
          ? `<tr>
        <td style="padding:10px 16px;color:#64748b;font-size:13px;font-weight:600">Hinweis</td>
        <td style="padding:10px 16px;color:#0f172a;font-size:14px">${note}</td>
      </tr>`
          : ""
      }
    </table>`;
  return {
    subject: `Ligazuordnung wartet auf Freigabe: ${teamName} → ${requestedLeagueName}`,
    html: emailLayout({
      baseUrl,
      accent: "orange",
      badge: "Freigabe nötig",
      title: "Ligazuordnung wartet auf Freigabe",
      intro: `${requesterName || "Ein Team-Admin"} möchte „${teamName}" der oben genannten Liga zuordnen. Das wirkt sich auf Tabelle, Spielplan und Statistiken aus – bitte prüfen.`,
      bodyHtml,
      ctaText: "Anfrage jetzt prüfen",
      ctaUrl: `${baseUrl}/admin/league-requests`,
      footerNote: "Interne Benachrichtigung · Hoops Germany Liga-Freigabe",
    }),
    text: `Ligazuordnung wartet auf Freigabe: ${teamName} → ${requestedLeagueName} (aktuell: ${currentLeagueName || "keine"}).${note ? `\nHinweis: ${note}` : ""}\nPrüfen: ${baseUrl}/admin/league-requests\n`,
  };
}

// ---------------------------------------------------------------------------
// 11 – Liga-Zuordnung genehmigt (an den anfragenden Team-Admin)
// ---------------------------------------------------------------------------
export function leagueChangeApprovedEmail({ teamName, leagueName, reviewNote, baseUrl }) {
  const bodyHtml = reviewNote
    ? `<p style="margin:0;color:#475569;font-size:14px;background:#f8fafc;border-radius:10px;padding:12px 16px">„${reviewNote}"</p>`
    : "";
  return {
    subject: `Ligazuordnung genehmigt: ${teamName} → ${leagueName}`,
    html: emailLayout({
      baseUrl,
      accent: "green",
      badge: "✅ Genehmigt",
      title: "Eure Ligazuordnung wurde genehmigt",
      intro: `„${teamName}" ist jetzt „${leagueName}" zugeordnet. Tabelle, Spielplan und Kader sind aktualisiert.`,
      bodyHtml,
      ctaText: "Zum Team-Panel",
      ctaUrl: `${baseUrl}/team/admin?tab=einstellungen`,
      footerNote: "Du erhältst diese E-Mail, weil du eine Ligazuordnung für dein Team angefragt hast.",
    }),
    text: `Eure Ligazuordnung wurde genehmigt: „${teamName}" → „${leagueName}".${reviewNote ? `\nHinweis: ${reviewNote}` : ""}\nTeam-Panel: ${baseUrl}/team/admin?tab=einstellungen\n`,
  };
}

// ---------------------------------------------------------------------------
// 12 – Liga-Zuordnung abgelehnt (an den anfragenden Team-Admin)
// ---------------------------------------------------------------------------
export function leagueChangeRejectedEmail({ teamName, leagueName, reviewNote, baseUrl }) {
  const bodyHtml = reviewNote
    ? `<p style="margin:0;color:#475569;font-size:14px;background:#f8fafc;border-radius:10px;padding:12px 16px">„${reviewNote}"</p>`
    : "";
  return {
    subject: `Ligazuordnung abgelehnt: ${teamName} → ${leagueName}`,
    html: emailLayout({
      baseUrl,
      accent: "red",
      badge: "Abgelehnt",
      title: "Eure Ligazuordnungs-Anfrage wurde abgelehnt",
      intro: `Die Anfrage für „${teamName}" zu „${leagueName}" wurde abgelehnt. Ihr könnt jederzeit eine neue Anfrage stellen.`,
      bodyHtml,
      ctaText: "Zum Team-Panel",
      ctaUrl: `${baseUrl}/team/admin?tab=einstellungen`,
      footerNote: "Du erhältst diese E-Mail, weil du eine Ligazuordnung für dein Team angefragt hast.",
    }),
    text: `Eure Ligazuordnungs-Anfrage wurde abgelehnt: „${teamName}" → „${leagueName}".${reviewNote ? `\nHinweis: ${reviewNote}` : ""}\nTeam-Panel: ${baseUrl}/team/admin?tab=einstellungen\n`,
  };
}
