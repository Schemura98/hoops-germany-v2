...
export async function deletePlayerCascade(playerId) {
  ...
  await Promise.all([
    ...
    // Beanspruchte Kader-Slots freigeben – über die gemeinsame Funktion, nicht
    // mehr als eigene Kopie der Abfrage (Fund von Kai, 13.08.2026): Die Kopie
    // hier löschte den `claimToken` NICHT, ließ also genau die
    // Wieder-Beitritts-Lücke offen, die `slotsFreigeben` schließt – ein leerer
    // Platz mit weiterhin gültigem Alt-Link. Ohne `teamId`-Argument werden alle
    // Vereine bereinigt; das ist beim Löschen eines Kontos richtig.
    slotsFreigeben(playerId),
