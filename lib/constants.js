...
// Platzhalter, wenn zu einer Person keine Position hinterlegt ist – EIN Text
// für alle Flächen (Wortlaut Nele, 15.08.2026).
//
// Vorher standen vier verschiedene Strings an neun Stellen und widersprachen
// sich: `—` auf der öffentlichen Vereinsseite, auf `/topscorer` und in der
// Liga-Tabelle, „Position offen" im Team-Admin-Panel, im Transfermarkt und in
// zwei Tabs, gar nichts auf `/spieler` und in der zweiten Liste des
// Transfermarkts.
// ⚠️ „Position offen" ist raus, weil es eine ausgeschriebene Stelle behauptet.
// Bei einem Mitglied ist das schlicht falsch; bei einem Kaderplatz auch, denn
// öffentlich sichtbar sind nur Plätze MIT Namen – ein erwartetes Mitglied ohne
// Konto, keine Vakanz. Im Transfermarkt liest es sich neben einem Spielernamen
// wie „spielt alles, sucht irgendwas".
// ⚠️ „—" ist überall raus. Meine erste Fassung nahm `/topscorer` und
// `/ligen/[id]` mit dem Argument aus, der Gedankenstrich sei dort
// „Tabellensprache in einer Spalte mit Kopfzeile". Das war falsch, und Kai und
// Tobias haben es unabhängig voneinander nachgemessen: An beiden Stellen steht
// die Position NICHT in einer eigenen Spalte, sondern als Unterzeile unter dem
// Spielernamen – zeichengleich mit `/transfermarkt`, das ich geändert hatte.
// Es gab keine Kopfzeile, auf die sich der Strich hätte beziehen können.
// Damit gilt der ursprüngliche Grund ohne Ausnahme: In einer Personenliste
// ohne Spaltenkopf kann ein Fremder nicht unterscheiden zwischen „keine
// Angabe", „keine Position" und „Ladefehler".
// ⚠️ Bewusst NICHT „nicht eingetragen": Das klingt nach einer Handlung, für die
// es im Panel keinen Knopf gibt – eine Slot-Position lässt sich nach dem
// Anlegen gar nicht mehr ändern.
//
// ⚠️ Und bewusst NICHT mehr „Position nicht angegeben" (Nele, 15.08.2026,
// wenige Stunden nach ihrer eigenen ersten Fassung). Der Grund ist inhaltlich,
// nicht die Breite: Der Satz benennt das fehlende Feld als POSITION – aber
// dasselbe Feld trägt auch `PLAYER_ROLES` (Coach, Manager, Sportliche Leitung,
// Fan). Neben einem Trainer behauptete er, es fehle eine Spielposition. Im
// Sinne des Codes richtig (`position` ist leer), im Sinne des Lesers falsch –
// also genau das Muster aus docs/MUSTER-ZAHLEN-DIE-LUEGEN, gegen das der Text
// geschrieben war. „Keine Angabe" sagt nur, dass nichts hinterlegt ist, und das
// stimmt für beide Sorten Mensch.
//
// Nebenwirkung, nicht Zweck: 75 px statt 138 px. Der Platzhalter ist damit
// kürzer als „Small Forward" und hört auf, der längste String des Vokabulars zu
// sein – der schlimmste Fall ist wieder echter Inhalt („Sportliche Leitung").
//
// ⚠️ Was die Kürzung NICHT löst: Die Kompaktliste im Transfermarkt (179 px)
// schneidet das Bundesland schon heute bei VIER von fünf Spielpositionen ab
// (`Point Guard · Nordrhein-Westfalen` = 188 px). Das ist ein Gestaltungs-
// befund für Vivien und bestand vor diesem Platzhalter. Wer hier weiter kürzt,
// behandelt das falsche Problem.
//
// Verworfene Kurzformen, damit sie nicht wiederkommen: „Ohne Position" /
// „Keine Position" (Aussage über die PERSON statt über die Daten – für
// Coach/Manager/Fan falsch), „Position fehlt" (Defektsprache), „k. A."
// (Abkürzung, in einer Personenliste nicht entzifferbar).
export const POSITION_FEHLT = "Keine Angabe";
