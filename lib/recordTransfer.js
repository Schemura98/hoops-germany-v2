...
function transferMessage(name, type, fromName, toName) {
  switch (type) {
    ...
    case "leave":
      // „hat verlassen" behauptete eine Handlung des Spielers – auch dann,
      // wenn ein Team-Admin ihn über `remove-member` entfernt hat (Befund Kai,
      // Wortlaut Nele, 14.08.2026). Im Feed stand damit, jemand habe ein Team
      // „verlassen", das er womöglich nie freiwillig verlassen hat.
      // „steht" statt „ist": Es beschreibt die Eintragung, nicht die
      // sportliche Wirklichkeit — „ist nicht mehr im Kader" klingt eine Spur
      // nach Rauswurf.
      // Die drei anderen Fälle bleiben in der Handlungsform: `found`, `join`
      // und `move` entstehen nur auf den sieben echten Wegen, bei denen der
      // Spieler tatsächlich gehandelt hat.
      return `${name} steht nicht mehr im Kader von ${fromName}.`;
    case "join":
    default:
