...
export async function computeAnalyticsSummary(periodInput) {
  ...
  const [
    ...
    AE.distinct("playerId", { playerId: { $ne: null }, eventType: { $ne: "own_stats_notified" }, createdAt: { $gte: d7 } }),
    AE.distinct("playerId", { playerId: { $ne: null }, eventType: { $ne: "own_stats_notified" }, createdAt: { $gte: d30 } }),
    AE.aggregate([
