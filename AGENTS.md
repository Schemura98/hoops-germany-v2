...
>   Postfächern ankamen, muss der User selbst bestätigen** (Codex kann keine externen Postfächer lesen) –
...
> - **Skill `.Codex/skills/league-catalog/`** (SKILL.md + `references/`: current-architecture / product-rules
...
> Commit-Hash, Roadmap pflegen). AGENTS.md ist die **kanonische, session-übergreifende Quelle** –
> das private Session-Gedächtnis ersetzt sie nicht. Dafür gibt es die Skill **`log-progress`**
> (`.Codex/skills/log-progress/`). So fehlt auch bei einem späteren Umbau der Live-Seite nichts.
>
...
> (`.Codex/skills/update-feedback-analytics/`).
>
...
> (`.Codex/skills/update-onboarding-surfaces/`).
...
- **Deployment durchgeführt** (Codex per SSH-Key auf den Hostinger-VPS `92.113.25.249`, Ubuntu 24.04):
...
- **Updates:** `cd /root/hoops-v2 && git pull && npm run build && pm2 restart hoops-v2`. VPS-Repo-Zugang via
  Deploy-Key (`~/.ssh/github_hoops`, SSH-Alias `github-hoops`). Codex-SSH-Key: `~/.ssh/hoops_vps` (lokal).
...
> Updates deployen: `cd /root/hoops-v2 && git pull && npm run build && pm2 restart hoops-v2` (Codex per `~/.ssh/hoops_vps`).
