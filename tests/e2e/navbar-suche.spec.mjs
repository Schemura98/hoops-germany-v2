...
test.describe("Such-Overlay – Auswege", () => {
  ...
  test("Escape verwirft auch den eingetippten Suchbegriff", async ({
    page,
  }) => {
    const dialog = await sucheOeffnen(page);
  ...
  test("Tab bleibt im Dialog – auch mit Trefferliste und rückwärts", async ({
    page,
  }) => {
    const dialog = await sucheOeffnen(page);
    ...
    let drin = await dialog.evaluate((box) =>
      box.contains(document.activeElement),
    );
    expect(
      drin,
      "der Fokus ist beim Vorwärtstabben aus dem Dialog gewandert",
    ).toBe(true);
    ...
    expect(
      drin,
      "der Fokus ist beim Rückwärtstabben aus dem Dialog gewandert",
    ).toBe(true);
  });

  test("Klick auf eine tote Stelle im Panel wirft den Fokus nicht hinaus", async ({
    page,
  }) => {
    ...
    const drin = await dialog.evaluate((el) =>
      el.contains(document.activeElement),
    );
    expect(
      drin,
      "nach einem Klick ins Leere führt Tab aus dem Dialog heraus",
    ).toBe(true);
  });

  test("nach dem Schließen kehrt der Fokus auf den Öffner zurück", async ({
    page,
  }) => {
    const dialog = await sucheOeffnen(page);
    await page.keyboard.press("Escape");
...
test.describe("Such-Overlay – Ligen (Ronjas R8)", () => {
  test("der Platzhalter nennt Ligen, nicht nur Spieler und Team", async ({
    page,
  }) => {
    const dialog = await sucheOeffnen(page);
  ...
  test("eine echte Liga ist über die Suche auffindbar", async ({
    page,
    request,
  }) => {
    ...
    test.skip(
      ligen.length === 0,
      "Keine Ligen in der Dev-DB – seed-demo.mjs läuft?",
    );
...
test.describe("Nachfiltern, sobald die Daten da sind", () => {
  ...
  test("Eingabe VOR dem Laden der Daten geht nicht verloren", async ({
    page,
  }) => {
...
test.describe("Trefferflächen der Chrome-Symbole (WCAG 2.5.8)", () => {
  ...
  test("jeder Icon-Knopf der öffentlichen Leiste ist groß genug (mobil)", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 390, height: 844 });
    await page.goto(SEITE);
    ...
    expect(anzahl, "keine beschrifteten Icon-Ziele gefunden").toBeGreaterThan(
      0,
    );
  ...
  for (const label of ["Suche öffnen", "Feedback geben"]) {
    test(`„${label}" misst mindestens ${MINDEST}×${MINDEST} px`, async ({
      page,
    }) => {
      await page.goto(SEITE);
  ...
  test("das × im offenen Overlay misst mindestens 24×24 px", async ({
    page,
  }) => {
