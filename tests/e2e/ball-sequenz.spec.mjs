...
function genauEiner(text, regex, was) {
  ...
  if (treffer.length === 0) throw new Error(`${was}: kein Treffer für ${regex}`);
  if (treffer.length > 1) {
    throw new Error(`${was}: ${treffer.length} Treffer für ${regex} – erwartet genau einer`);
  }
  return treffer[0];
}
...
test.describe("Hero-Ball-Sequenz – die drei Kopplungsstellen", () => {
  test("Konstante, CSS-Dateinamen und die echte Datei nennen dieselbe Bildzahl", () => {
    ...
    const namen = [...css.matchAll(/ball-basketball-(\d+)x(\d+)\.(webp|avif)/g)];
    ...
    expect(Number(namen[1][2]), "WebP und AVIF nennen verschiedene Kantenlängen").toBe(kante);
  ...
  test("die Vorgabewerte des Erzeugers ergeben genau das ausgelieferte Asset", () => {
    ...
    const frames = Number(
      genauEiner(skript, /^const FRAMES = Number\(arg\("frames", (\d+)\)\);$/gm, "Erzeuger")[1]
    );
    const groesse = Number(
      genauEiner(skript, /^const GROESSE = Number\(arg\("groesse", (\d+)\)\)/gm, "Erzeuger")[1]
    );
    ...
    const css = lies("app/globals.css");
    const namen = [...css.matchAll(/ball-basketball-(\d+)x(\d+)\.(?:webp|avif)/g)];
    expect(namen.length, "globals.css nennt die Sequenz gar nicht").toBeGreaterThan(0);
    for (const [voll, cssFrames, cssKante] of namen) {
      expect(frames, `Vorgabe --frames passt nicht zu ${voll}`).toBe(Number(cssFrames));
      expect(groesse, `Vorgabe --groesse passt nicht zu ${voll}`).toBe(Number(cssKante));
    }
  });
  ...
  test("der Drehpunkt des Streckenballs ist nicht von außen überschreibbar", () => {
    ...
    ).toMatch(/\.\.\.style,\s*transformOrigin: `\$\{RAIL_BALL_R\}px \$\{RAIL_BALL_R\}px`/);
