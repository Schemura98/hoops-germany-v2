  ...
  return (
    <div className="min-h-screen bg-navy-950 flex flex-col">
      ...
      <PageHeader
        ...
        subtitle="Vereine und Teams mit Kader, Spielplan und gemeldeten Ergebnissen – aus deiner Liga und deinem Kreis."
      >
        <div className="mt-5 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
          ...
          {/* „automatisch" stand hier zu Unrecht: Das Team entsteht mit
              approved:false und ist erst nach Freigabe öffentlich sichtbar –
              der Gründer erfuhr das bisher erst NACH dem Absenden
              (Befund Nele, 23.08.2026). */}
          <p className="text-mist-400 text-sm">
            Dein Team ist nicht dabei? Gründe es selbst – du wirst{" "}
            <span className="text-mist-300 font-medium">Team-Admin</span> und kannst sofort
            Kader, Spiele und Tryouts einrichten. Öffentlich sichtbar wird das Team nach
      ...
      <main id="hauptinhalt" tabIndex={-1} className="flex-1 max-w-5xl mx-auto w-full px-4 py-8">
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            text={
              query || land || geo.center
                ? "Keine Teams für diese Suche oder Filter – Auswahl anpassen."
                : "Noch keine Teams registriert – gründe das erste."
            }
            action={
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {filtered.map((t, i) => (
              <Reveal key={t._id} delay={staffel(i)} className="h-full">
              <Link
                ...
                data-vt
                className="group h-full bg-navy-800 rounded-md border border-navy-600 hover:border-brand-500 hover:bg-navy-700 transition-[background-color,border-color] duration-200 ease-out-strong overflow-hidden flex flex-col"
              >
                {/* Flaches navy-900 statt Verlauf: Die Spezifikation verbietet
                    Verläufe ausdrücklich („auch keine dunklen") und ordnete für
                    genau diese Kachel „flach, kein Verlauf" an (§4.2). Es war
                    der einzige Verlauf aller Spieler-/Vereinsseiten. */}
                <div className="bg-navy-900 flex items-center justify-center h-40 w-full">
                  {t.logo ? (
